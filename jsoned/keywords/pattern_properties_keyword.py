...
class PatternPropertiesKeyword(AssertionKeyword):
    ...
    def apply(self, schema: JsonSchema, node: JsonObject, validator: ValidatorsMap):
        ...
        pattern_properties_validators = ValidatorsMap[Pattern]()

        for key, value in node[self.key].items():
            pattern_properties_validators[re.compile(key)] = partial(
                deferred_validator,
                schema=schema,
                node=value,
            )
        ...
        if "object_properties" in validator:
            ...
            object_properties_validator = partial(
                object_properties,
                pattern_properties=pattern_properties_validators,
            )
        else:
            object_properties_validator = partial(
